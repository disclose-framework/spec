...
The standard attribute set is designed to address the three core risks an agent must evaluate before recommending a purchase:
...
| `disclose:product_return_rate` | decimal | Rate of units returned across all orders (0–1). May be disclosed at SKU or category level. |
...
| `disclose:refund_processing_time_median_days` | decimal | Median business days from return receipt to refund completion |
| `disclose:refund_processing_time_p90_days` | decimal | 90th percentile business days from return receipt to refund completion |
...
- **Subjective & Qualitative signals** (see Standard Attributes) should be distinguished from operational metrics when surfaced to buyers. Agents **SHOULD** treat `disclose:review_rating` as contextual rather than authoritative, and **SHOULD** surface `disclose:review_verified_purchase_rate` alongside it wherever possible.
